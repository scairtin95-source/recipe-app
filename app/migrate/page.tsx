...
export default function MigratePage() {
  ...
  const runBatch = async () => {
    const res = await fetch('/api/migrate-recipes?limit=3')
  ...
  const start = async () => {
    ...
    try {
      ...
      while (remainingCount > 0 && !stopRef.current) {
        await new Promise((r) => setTimeout(r, 1200)) // brief pause between batches
    ...
    <div style={{ minHeight: '100vh', background: COLORS.neutral, fontFamily: 'var(--font-manrope)' }}>
      <main style={{ maxWidth: 700, margin: '0 auto', padding: '3rem 1.5rem' }}>
        ...
        <p style={{ color: '#8a8378', fontSize: '0.9rem', margin: '0 0 1.5rem' }}>
          Re-parses recipes saved before structured ingredients existed — backfills quantities/units, prep/cook/total time, servings, and missing images (re-fetches each recipe's source URL, 3 at a time).
        </p>
        ...
        {totalAtStart !== null && (
          <div style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: COLORS.secondary, marginBottom: '0.4rem' }}>
              <span>{totalAtStart - (remaining ?? 0)} of {totalAtStart} processed</span>
            ...
            {[...log].reverse().map((entry, i) => (
              <div key={`${entry.id}-${i}`} style={{
                ...
                <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: '1rem' }}>
                  <span style={{ color: entry.status === 'ok' ? COLORS.secondary : COLORS.primary, fontWeight: 700, marginRight: '0.5rem' }}>
                    {entry.status === 'ok' ? '✓' : '✕'}
                  </span>
                  {entry.title || `Recipe #${entry.id}`}
                  {!entry.usedSourceUrl && (
                    <span style={{ color: '#8a8378', fontSize: '0.75rem', marginLeft: '0.5rem' }}>(no URL — ingredients only)</span>
                  )}
                </div>
                {entry.error && (
